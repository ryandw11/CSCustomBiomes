...
public class CustomBiomeAddon_1_18 implements StructureSection {
    ...
    public boolean checkStructureConditions(Structure structure, Block block, Chunk chunk) {
        ...
        try {
            ...
            Biome biomeBase = worldServer.getBiome(craftBlock.getPosition());

            DedicatedServer dedicatedServer = ((CraftServer) Bukkit.getServer()).getServer();

            WritableRegistry<Biome> registry = dedicatedServer.registryAccess().ownedRegistry(Registry.BIOME_REGISTRY).get();
            ...
            if (category != null) {
                if (!category.equalsIgnoreCase(biomeBase.getBiomeCategory().getName())) return false;
            }
